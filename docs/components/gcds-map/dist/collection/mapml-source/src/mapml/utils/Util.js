      ...
      switch (link.target) {
        ...
        case '_parent':
          ...
          for (let l of map.options.mapEl.querySelectorAll('map-layer,layer-'))
            if (l._layer !== leafletLayer && l.parentNode === map.options.mapEl) 
              map.options.mapEl.removeChild(l);
          map.options.mapEl.appendChild(layer);
          if (leafletLayer._layerEl && leafletLayer._layerEl.parentNode === map.options.mapEl)
