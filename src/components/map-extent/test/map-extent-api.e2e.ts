...
test.describe('map-extent tests', () => {
  ...
  test.beforeAll(async function () {
    ...
    await page.goto('/test/map-extent/map-extent-api.html', { waitUntil: 'networkidle' });
  ...
  test('map-extent.getMapEl() works', async () => {
    ...
    await map.evaluate((map) => {
      map.insertAdjacentHTML(
        'beforeend',
        `<map-layer data-testid="remote-layer" src="/test/map-extent/remote-layer.mapml"></layer>`
      );
    });
    ...
    await page.goto('/test/map-extent/map-extent-api-web-map.html', { waitUntil: 'networkidle' });
    await page.waitForTimeout(1000);
