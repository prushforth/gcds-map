...
test.describe('Playwright templatedPMTilesLayer Tests', () => {
  ...
  test('Test rendered data', async ({ page }) => {
    await page.goto('/test/map-layer/templatedPMTilesMVTLayer.html',  { waitUntil: 'networkidle' });
    ...
    await viewer.evaluate((v: any) => v.zoomTo(0, 0, 2));
    await expect(viewer).toHaveScreenshot('mvt-light-z2.png', {
      maxDiffPixels: 1500
    });
    const lightLayer = page.getByTestId('light');
    await lightLayer.evaluate((l: any) => l.removeAttribute('checked'));
    await viewer.evaluate((v: any) => v.reload());

    await page.waitForTimeout(3000);
    const darkLayer = page.getByTestId('dark');
    await darkLayer.evaluate((l: any) => l.setAttribute('checked', 'checked'));
    await expect(viewer).toHaveScreenshot('pmtiles-dark.png', {
      maxDiffPixels: 1500
    });
    await darkLayer.evaluate((l: any) => l.zoomTo());
  ...
  test('pmtilesStyles.js module not found console errors', async ({ page }) => {
    ...
    await page.goto('/test/map-layer/templatedPMTilesMVTLayerMissingStyles.html',  { waitUntil: 'networkidle' });
    await page.waitForTimeout(1000);
    ...
    messages.length = 0;
    await page.goto('/test/map-layer/templatedPMTilesMVTLayerMissingRuleKey.html',  { waitUntil: 'networkidle' });
    await page.waitForTimeout(1000);
  ...
  test('A protomaps map-link in a non-OSMTILE map-extent is never enabled', async () => {
    await page.goto('/test/map-layer/templatedPMTilesCBMTILETest.html',  { waitUntil: 'networkidle' });
    ...
    const viewer = page.getByTestId('viewer');
    await viewer.evaluate((v: any) => (v.projection = 'OSMTILE'));
    await page.waitForTimeout(500);
  ...
  test('A protomaps map-link, parent map-extent and ancestor map-layer are disabled when out of bounds', async ({
    ...
    const darkLayer = page.getByTestId('dark');
    await lightLayer.evaluate((l: any) => l.removeAttribute('checked'));
    await darkLayer.evaluate((l: any) => {
      l.checked = 'checked';
      l.zoomTo();
    });
  ...
  test('A protomaps mvt map-link that has {z},{x},{y} variable names without associated map-inputs will not render', async ({
    ...
    await lightLayer.evaluate((l: any) => l.removeAttribute('checked'));
    const darkLayer = page.getByTestId('dark');
    await darkLayer.evaluate((l: any) => l.removeAttribute('checked'));
    const hardCodedVariablesLayer = page.getByTestId('hard-coded-variables');
    await hardCodedVariablesLayer.evaluate((l: any) => (l.checked = true));
    await page.waitForTimeout(1000);
  ...
  test('Custom pmtilesRules can render mvt data', async ({ page }) => {
    await page.goto('/test/map-layer/templatedPMTilesMVTLayerCustomStyles.html',  { waitUntil: 'networkidle' });
    await page.waitForTimeout(1000);
    const viewer = page.getByTestId('viewer');
    // seems like a lot of pixels to allow to be different, but missing fonts
    // across instances can require the snapshots to tolerate differences
