...
test.describe('Playwright gcds-map Context Menu (and api) Tests', () => {
  let page;
  let context;
  ...
  test('Submenu, copy map (MapML)', async () => {
    ...
    const expected = `<gcds-map data-testid="testviewer" style="height: 600px;width:500px;" projection="CBMTILE" zoom="0" lat="47" lon="-92" controls="" role="application">
  ...
  test('Submenu, copy gcds-map location in gcrs (default) coordinates', async () => {
  ...
  test('Paste map-feature to gcds-map', async () => {
  ...
  test('Context menu, All buttons enabled when fwd and back history present', async () => {
    await page.click('body > gcds-map');
    await page.$eval('body > gcds-map', (map) => map.zoomTo(81, -63, 3));
    await page.waitForTimeout(1000);
    await page.$eval('body > gcds-map', (map) => map.zoomTo(81, -63, 5));
    await page.waitForTimeout(1000);
    await page.click('body > gcds-map', { button: 'right' });
    await page.click('div > div.mapml-contextmenu > button:nth-child(1)');
    await page.click('body > gcds-map', { button: 'right' });
