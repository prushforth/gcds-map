...
test.describe('map-feature rendering tests', () => {
  ...
  test('removing a map-feature from DOM removes its rendering', async () => {
    ...
    const rendered = await f.evaluate((f) => {
      (f as any)._featureLayer._container.setAttribute(
        'data-testid',
        'test-feature-container'
      );
      (f as any)._groupEl.setAttribute('data-testid', 'test-feature-rendering');
      return (f as any)._groupEl.isConnected;
    });
  ...
  test('removing last map-feature in a sequence removes rendering container', async ({
    ...
    const containerConnected = await viewer.evaluate((v) => {
      (v.querySelector('map-feature') as any)._featureLayer._container.setAttribute(
        'data-testid',
        'test-feature-container'
      );
      return (v.querySelector('map-feature') as any)._featureLayer._container
        .isConnected;
    });
